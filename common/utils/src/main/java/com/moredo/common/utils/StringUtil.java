...
StringUtil {
    ...
    private  static final  String xhxConvert(String name) {

        String[] split = name.split("_");
        String _name = "";
        if (split.length > 0) {
            for (int i = 0; i < split.length; i++) {
                _name += initcap(split[i]);
            }
        }
        return _name;
    }

    public static final  String filterEmoji(String filter){
        Pattern emoji = Pattern.compile("[\ud83c\udc00-\ud83c\udfff]|[\ud83d\udc00-\ud83d\udfff]|[\u2600-\u27ff]", Pattern.UNICODE_CASE | Pattern.CASE_INSENSITIVE ) ;
        Matcher matcher = emoji.matcher(filter);
        while (matcher.find()){
            filter = filter.replace(matcher.group(), "");
        }
        return filter;
    }
}
